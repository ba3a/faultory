...
| `core.shop.systems` | Per-responsibility simulation systems extracted from `ShopFloor`: `ConveyorSystem`, `ProductionSystem`, `QaSystem`, `WorkerObjectiveSystem`, `WorkerMovementSystem`, `SecuritySystem`, and the rest. Each implements `SimulationSystem` and declares a `SimulationPhase`; `SimulationSchedule` orders them by phase and `ShopFloor` (a thin facade) runs the schedule. The execution-order contract — what each phase does and why it sits where it does — is the KDoc on `SimulationPhase`. `PlacementSystem` and `AssignmentSystem` also live here but are **command handlers** the UI calls through the facade (placement/rotation/upgrade, worker assignment) — not scheduled, no `SimulationPhase`. `ShopFloorState` holds the whole mutable world; **a system never receives it directly** — it takes exactly one narrow `*Access` interface from `ShopFloorAccess.kt` (`ConveyorAccess`, `QaAccess`, …, each composed of capability interfaces like `PlacedObjectWrites` / `OccupancyReads`), and `ShopFloorState` is the sole implementation of all of them. Cross-system helpers are their own collaborators, not calls between systems: `QaPostLocator` (belt-side QA geometry, shared by `QaSystem` / `WorkerObjectiveSystem` / `AssignmentSystem`), `InteractionController` + `InteractionStarter` (`InteractionSystem` is a thin scheduled wrapper; `CleanerSystem` starts handoffs through the interface), `WetFloor` + `WetFloorReads` / `WetFloorWrites` (puddle map; `WetTileSystem` only dries). |
...
- Do not give a system `ShopFloorState` (or a wide slice of it). Add or reuse a capability
  interface in `ShopFloorAccess.kt`, fold it into that system's `*Access` interface, and take the
  `*Access` — one narrow constructor param. `ShopFloorState` implements every `*Access`. Do not let
