...
enum class MessageKey(val path: String) {
    ...
    WORKER_DISPLAYNAME("catalog.workers.displayName"),
    MACHINE_DISPLAYNAME("catalog.machines.displayName"),
    PRODUCT_DISPLAYNAME("catalog.products.displayName"),
    LEVEL_DISPLAYNAME("catalog.levels.displayName"),
    LEVEL_SUBTITLE("catalog.levels.subtitle");

    val isCatalog: Boolean get() = path.startsWith(CATALOG_PREFIX)
